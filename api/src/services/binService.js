...
class BinService {
   ...
   * @returns {Promise<Object>} Updated bin
   */
  async updateFillLevel(binId, updateData) {
    try {
      const bin = await Bin.findOne({ id: binId, isDeleted: { $ne: true } });
      if (!bin) {
        throw new RecordNotFoundError('Bin');
      ...
      if (updateData.fillLevel !== undefined) {
        ...
        bin.currentFillLevel = updateData.fillLevel;
      }
      ...
      if (updateData.weight !== undefined) {
        ...
        bin.currentWeight = updateData.weight;
      }

      await bin.save();

      logBusinessEvent('bin_fill_level_updated', {
        binId,
        fillLevel: updateData.fillLevel,
        weight: updateData.weight,
      });

      return bin;
    } catch (error) {
      logError(error, { operation: 'updateFillLevel', binId, updateData });
      throw error;
    }
  }
  ...
  /**
   * Get full bins (above threshold)
   * @param {Object} filters - Filter criteria
   * @param {Object} pagination - Pagination options
   * @returns {Promise<Object>} Paginated full bin results
   */
  async getFullBins(filters = {}, pagination = {}) {
    try {
      const { page = 1, limit = 10 } = pagination;
      ...
      logError(error, { operation: 'getFullBins', filters, pagination });
      throw error;
    }
  }
   ...
   * @returns {Promise<Object>} Paginated bin results
   */
  async getBinsByCategory(category, pagination = {}) {
    try {
      const { page = 1, limit = 10 } = pagination;
      
      const validCategories = ['ORGANIC', 'RECYCLABLE', 'GENERAL', 'HAZARDOUS', 'E_WASTE', 'BULKY'];
      ...
      logError(error, { operation: 'getBinsByCategory', category, pagination });
      throw error;
    }
  }
   ...
   * @returns {Promise<Object>} Paginated bin results
   */
  async getBinsNearLocation(locationData, pagination = {}) {
    try {
      const { page = 1, limit = 10 } = pagination;
      const { longitude, latitude, radius = 1000 } = locationData;
      ...
      logError(error, { operation: 'getBinsNearLocation', locationData, pagination });
      throw error;
    }
  }
